...
MeteorOSUserManager.prototype.addFile = function(cwd, fsFile) {
    var self = this;
    MeteorOS_FS.insert(fsFile, function(err, fileObj) {
        if (!err) {
            ...
            var filePath = self.getPath(cwd) + '.files';

            var update = {}
            update['fs.' + filePath] = file;

            UserCollection.update(self.user._id, {
                $push : update
            });
        } else {
            ERROR('Error inserting file ' + fsFile.name() + '!');
        }
    });
}

MeteorOSUserManager.prototype.removeFile = function(cwd, fileId) {
    // Remove from the file system
    MeteorOS_FS.remove(fileId);
    ...
    var path = this.getPath(cwd);

    var update = {}
    update['fs.' + path + '.files'] = files;
...
MeteorOSUserManager.prototype.renameFile = function(cwd, fileId, fileName) {
    var files = this.traverseFileTree(cwd).files;
    for (var i = 0; i < files.length; i++) {
        if (files[i].id == fileId) {
